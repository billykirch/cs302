# Starter code taken from diffTaichi: https://github.com/taichi-dev/difftaichi.git

import test_scenes as scenes
import taichi as ti
import argparse
import os
import math
...
### See line 290 for appended code ###
...
class Scene:
  ...
  # Add circle function created by billykirch  
  def add_circle(self, x, y, r, actuation, ptype=1):
        ...
        r_count = int(r / dx) * 2
        real_dr = r / r_count
        for i in range(-r_count, r_count): # (x,y) is center, encorporate -r_count
            for j in range(-r_count, r_count):
                # particle positions
                px = x + (i + 0.5) * real_dr + self.offset_x
                py = y + (j + 0.5) * real_dr + self.offset_y
                # add particles to scene if inside circle radius
